...
Last updated: 2026-08-31, after an on-device pass on the design work below (Pixel emulator, both themes, `font_scale 1.3`) which found three defects that only running it could surface, all now fixed:

1. **The paywall's mineral field inverted between themes** — the same class of bug the colorize pass existed to fix, reintroduced by the paywall itself. `$green9` is a *step*, a distance from its own theme's ground, not a brightness: it is L31% on a white page and L51% on a near-black one. At badge size that reads as one colour; across half a screen it was a calm block in light and a glare in dark (measured 4.0× darker than the page in light, 8.5× brighter in dark). It also could not carry text — even pure white on light `$green9` is 4.01:1, under the body minimum, and the fine print was at 3.42:1. The field is now composed per theme (`FieldPalette` in `PaywallScreen.tsx`): `$green11` light / `$green6` dark, landing at almost the same absolute luminance (0.119 / 0.096), with every text pair on it clearing 4.5:1.
2. **The paywall's CTA fell below the fold at `font_scale 1.3`** — the one screen that cannot hide its own button. The CTA, the renewal note and the restore/legal row are now a pinned footer on the field's own ground, so only the argument scrolls.
3. **Onboarding's answer options looked disabled** — the assessment's four options were plain Tamagui `Button`s and rendered grey-on-grey; the only question in the flow did not look answerable. They now use the same `OptionRow` as the two steps before them. The "Başlangıç Değerlendirmesi" heading also dropped its green: green means "there is an action here", and a heading is not one.

Prices are set in Play Console, not in code — the paywall renders whatever `product.priceString` the store returns. The intended figures (₺89,99 / ₺899,99 in Turkey, €4,99 monthly elsewhere) and what the screen will compute from them are recorded in `RELEASE_TODO.md` § 1 and pinned by tests in `pricing.test.ts`.

Before that: 2026-08-31, after a full design pass (`/impeccable` audit → harden → polish → distill → colorize → adapt → clarify) and replacing the hosted paywall with a custom one.
...
- Run the legal site's first `wrangler deploy` so `legal/public/` becomes the live source — see `RELEASE_TODO.md` § 4
...
2. Produce a production build and smoke-test the full path on a physical device: onboarding → exercise → completion → statistics → paywall → sandbox purchase → settings (full checklist in `RELEASE_TODO.md` § 6).
