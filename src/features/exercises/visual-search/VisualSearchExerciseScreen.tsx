...
export function VisualSearchExerciseScreen({ timeLimitMs, onComplete }: VisualSearchExerciseScreenProps) {
    ...
    return (
      <YStack f={1} bg="$background" jc="center" ai="center" p="$4" gap="$4">
        <Text fontSize="$8" fontWeight="bold" color="$green10">
          {t('visualSearch.completed', { ns: 'exercises' })}
        </Text>
  ...
  const gridLayout = computeGridLayout(width, cols, {
    ...
    rows: Math.ceil(gridWords.length / cols),
    // Word chips are half-height, so a row costs half a cell width.
    cellAspect: 0.5,
  });
  ...
  const itemWidth = gridLayout.cellSize;
              ...
              <YStack gap="$4" ai="center" w="100%">
                ...
                <View style={{
                  ...
                  {gridWords.map((word, i) => (
                    <View key={i} style={{ width: itemWidth, height: itemHeight, margin: gridGap / 2 }}>
                      <Button 
                        w="100%" 
                        h="100%" 
                        p={0} 
