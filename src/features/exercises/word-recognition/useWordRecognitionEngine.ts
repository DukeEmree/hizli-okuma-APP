...
export interface WordRecognitionConfig extends Partial<ExerciseConfig> {
  timeLimitMs: number;
}
...
export function useWordRecognitionEngine(config: WordRecognitionConfig, onCompleteCallback?: (result: ExerciseResult) => void) {
  ...
  const engine = useExerciseEngine(wordRecognitionDefinition, config, handleComplete);

  const generateNewTarget = useCallback(() => {
    const targetIdx = Math.floor(Math.random() * wordList.length);
    const target = wordList[targetIdx];
    
    // Choose false options depending on difficulty
    const numOptions = Math.min(6, 3 + Math.floor(engine.session.currentDifficulty / 3)); // 3 to 6 options
    let falseOptions: string[] = [];
    while (falseOptions.length < numOptions - 1) {
